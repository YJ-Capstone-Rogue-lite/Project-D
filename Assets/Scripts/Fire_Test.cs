...
public class FIre_Test : MonoBehaviour
{
    ...
    public bool isReloading = false; // 재장전 중인지 여부를 나타내는 변수
    ...
    {
        // 마우스 왼쪽 버튼을 누르고 발사 타이머가 0보다 작고, 활성화된 장탄수가 0보다 크고, isReloading이 false이고 무기가 할당되어 있을 때 총알 발사
        if (Input.GetMouseButton(0) && fireTimer <= 0f && (weapon_Slot.magazineCapacitySlot1 > 0 || weapon_Slot.magazineCapacitySlot2 > 0) && isReloading == false && weapon != null) //지금 둘중 하나라도 0보다 크면 총이 발사된다. 나중에 수정함
        {
            Debug.Log(isReloading + ", " + (weapon_Slot.magazineCapacitySlot1 > 0) +  ", " + (weapon_Slot.magazineCapacitySlot2 > 0));
